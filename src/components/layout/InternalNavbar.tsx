...
export function InternalNavbar() {
  ...
  return (
    <nav className="no-print fixed top-0 left-0 right-0 z-50 flex h-[60px] items-center justify-between bg-charcoal px-3 shadow-md sm:px-6">
      {/* Brand */}
      <Link href={`/${project.slug}/internal`} className="shrink-0">
      ...
      <div className="flex items-center gap-2">
        <span className="rounded-full bg-navy px-3 py-1 text-[11px] font-semibold tracking-wide text-warm-white/80">
          Internal Dashboard
        </span>
      </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-3">
        ...
        {/* User menu with admin link tucked inside */}
        <UserButton
          appearance={{
            elements: {
              avatarBox: "h-7 w-7",
